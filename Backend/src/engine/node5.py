...
2. 전략 도구 직접 호출(compare_supply_strategy, calculate_winning_probability,
   ...
   유발했음. 자세한 배경은 docs/fix-log.md Fix 4 참고.)
...
[결론 작성 규칙]
- "신청 자격"(특공/일반공급 조건, 위 [자격 확정 여부] 내용)과 "1순위 자격"(위 [청약 시점(1순위 자격) 분석 결과] 기준)은 서로 다른 개념입니다. 두 자격을 같은 단어("자격")로 섞어 쓰지 말고, 동사를 다르게 써서 명확히 구분하세요. "자격은 충족했으나 자격은 충족하지 못했다"처럼 같은 단어가 반복되는 문장은 절대 쓰지 마세요. 예: "청약 신청은 가능하나, 1순위 자격(가입기간/납입횟수)은 아직 충족하지 못했습니다."
