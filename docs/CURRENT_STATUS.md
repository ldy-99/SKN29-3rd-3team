...
| PDF 분석 | 미완료 | React 화면과 Django proxy는 있으나 FastAPI endpoint 없음 |
...
flowchart LR
    ...
    pdf["PDF 분석<br/>/api/pdf/analyze"] -. "현재 FastAPI endpoint 없음" .-> fastapi
    ...
    class react,django,fastapi,pipeline,rag ok;
    class user base;
    class pdf warn;
```
...
- 현재 MVP 정상 경로는 PDF가 아니라 수동 공고문 입력 또는 기본 프로필 진단이다.
...
flowchart TD
    ...
    profile -. "PDF 화면은 있으나" .-> pdf["PDF 업로드 분석"]
    pdf -. "FastAPI endpoint 미구현" .-> blocked(["현재 정상 경로 제외"])
    ...
    class pdf,blocked warn;
    class start,choose point;
```

PDF 업로드 화면은 있으나 아직 정상 MVP 경로에 포함하지 않는다.
...
| PDF 업로드 분석 | 미완료 | FastAPI `/api/pdf/analyze` 구현 필요 |
...
현재 마지막 FastAPI endpoint가 없다. 따라서 PDF는 “UI/proxy 준비, AI 분석 endpoint 미완료” 상태다.
...
| `frontend-react/src/app/pages/PdfAnalysis.tsx` | PDF 업로드 화면, 현재 미완료 경로 |
...
| 2 | PDF 범위 결정 | MVP 제외 유지 또는 FastAPI endpoint 구현 결정 필요 |
