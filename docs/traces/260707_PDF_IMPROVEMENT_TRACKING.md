...
- 2026-07-07: 전략 진단 화면에서 바로 PDF 파일 선택/드래그가 가능하도록 연결했다. PDF 처리 로딩 UI는 기존 `ProcessingIndicator`를 재사용하고, 분석된 정리본과 PDF 메타데이터는 브라우저 탭 세션에 임시 저장해 다른 탭에 다녀와도 사라지지 않게 했다.
- 2026-07-07: 긴 정책 안내문이 공고 제목으로 잡히는 케이스를 줄이기 위해 제목 정리 규칙을 보강했다.
- 2026-07-07: 원격 브랜치에 추가된 프론트 진행 UI를 확인한 뒤 전략 진단 화면의 PDF 진입점을 하나로 줄였다. 상단 버튼과 내부 파일 선택 버튼이 동시에 보여 혼란이 있었고, 드래그 드롭 이벤트가 안정적으로 전달되지 않아 업로드 영역 전체를 클릭/드롭 가능한 단일 영역으로 정리했다.
- 2026-07-07: PDF 추출 중/최종 진단 중 진행창은 `현재 지원 범위` 안내 아래가 아니라 공고 입력 카드 내부에서 입력 영역을 대체하도록 조정했다. 사용자가 현재 작업 중인 맥락을 잃지 않게 하고, 로딩 중 불필요한 입력 UI를 숨기기 위한 변경이다.
- 2026-07-07: PDF 드래그 중에는 `아파트 분양 모집공고 입력` 카드 전체가 드롭 영역처럼 보이도록 오버레이를 추가했다. 사용자가 작은 업로드 박스를 정확히 겨냥하지 않아도 카드 안 어디든 놓을 수 있게 하기 위한 UX 보강이다.
- 2026-07-07: PDF 추출 로딩과 최종 진단 로딩의 시각 구조를 통일했다. 현재 백엔드가 단계별 진행 이벤트를 보내지 않으므로 진행률 바는 실측값이 아니라 `파일 확인 -> 내용 추출 -> 결과 정리`, `프로필 확인 -> 공고 조건 비교 -> 전략 정리` 단계별 시간 추정값으로 표시한다.
- 2026-07-07: 드래그 이벤트는 카드 전체 영역만 담당하도록 정리했다. 내부 업로드 박스의 중복 drag handler를 제거해 enter/leave depth 계산이 흔들릴 여지를 줄였다.
- 2026-07-07: 레포트 제목은 표시 시점 계산만 하지 않고 `input_snapshot.announcement.display_title`로 명시 저장하도록 보강했다. PDF 기반 진단은 추출된 공고명/파일명/입력 텍스트 순서로 제목을 정하고, 공고 없는 진단은 `청약 가능성 분석`으로 저장한다.
