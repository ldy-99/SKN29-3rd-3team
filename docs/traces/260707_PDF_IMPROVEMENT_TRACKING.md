# PDF 개선 작업 추적

기준일: 2026-07-07
작업 브랜치: `integrate-eunjin-v2-0707`

## 1. 시작 상태

- `integrate-eunjin-v2-0707`은 `origin/final` 기준 2커밋 앞선 PR 대기 브랜치다.
- PDF MVP는 이미 구현되어 있다.
- 현재 흐름은 PDF 원본 저장 없이 텍스트/표를 추출하고, 추출 결과 `combined_text`를 React에서 확인한 뒤 전략 진단의 `announcement_text`로 넘긴다.
- 문제는 `combined_text`가 원문 추출에 가까워 사용자 검토 가치가 낮고, 진단 입력에도 불필요한 잡음이 많다는 점이다.

## 2. Baseline: 개선 전 PDF 처리 로직

작업 시작 시점의 기존 로직이다. 이후 변경사항과 비교하기 위한 기준선으로 둔다.

```text
React PdfAnalysis.tsx
  -> api.analyzePdf(file)
  -> Django PDFAnalyzeAPIView
      - 인증 필요
      - PDF 확장자/MIME 검사
      - 15MB 제한
      - 원본 저장 없이 bytes read
  -> FastAPI /api/pdf/analyze
      - PDF 확장자/MIME 검사
      - 빈 파일/15MB/%PDF 헤더 검사
  -> Backend/app/services/pdf_service.py
      - pdfplumber로 모든 페이지 텍스트 추출
      - pdfplumber로 모든 표 추출
      - 텍스트가 500자 미만이면 PyMuPDF fallback
      - 본문 앞 12,000자 + 표 최대 30개를 합쳐 combined_text 생성
      - combined_text 15,000자 초과 시 잘라냄
  -> React에서 preview/combined_text 표시
  -> 전략 진단 버튼 클릭
  -> StrategyRun.tsx state로 combined_text 전달
  -> POST /api/strategy announcement_text로 전송
  -> Django StrategyRun input_snapshot/result_payload 저장
  -> FastAPI /api/announcement가 Node4 LLM 구조화 후 상세 진단 실행
```

### Baseline 한계

- PDF가 길거나 표가 많으면 모든 페이지/표를 훑어 추출 시간이 길어질 수 있다.
- 사용자가 보는 내용이 raw 추출문에 가까워 “무엇을 확인하고 수정해야 하는지”가 명확하지 않다.
- 반복 안내문, 목차, 긴 유의사항, 표 깨짐 텍스트까지 함께 들어가 진단 입력 품질을 떨어뜨릴 수 있다.
- React에서 `combined_text`는 보여주지만, 진단 입력으로 사용할 공고 요약본과 원문 추출 일부가 분리되어 있지 않다.
- `pdf_analysis_id`가 PDF 분석 응답에는 있으나 전략 진단 요청까지 이어지지 않아 추후 이력 추적성이 약하다.
- 마이페이지/결과 상세에서 “어떤 PDF 공고를 기준으로 진단했는지”는 `source_filename`과 입력 스냅샷에 기대고 있으며, 정리된 공고문/요약본 저장 기준이 아직 명확하지 않다.

## 3. 개선 필요성

이번 개선은 단순히 PDF 텍스트를 더 많이 뽑는 것이 아니라, **진단에 쓸 수 있는 공고문 입력값을 더 빠르고 읽기 좋게 만드는 것**이 목적이다.

사용자 관점:

- raw 추출문을 그대로 보여주면 수정할 지점을 찾기 어렵다.
- 공고명, 공급 위치, 분양가, 청약 일정, 제한사항처럼 확인해야 할 항목이 먼저 보여야 한다.
- 나중에 마이페이지에서 내가 어떤 공고 기준으로 진단했는지 다시 이해할 수 있어야 한다.

진단 성능 관점:

- Node4는 자유 텍스트에서 공고 구조를 LLM으로 추출하므로, 입력값이 지저분할수록 구조화 품질이 흔들릴 수 있다.
- PDF 추출 단계에서 불필요한 잡음을 줄이고 핵심 항목 중심으로 정리하면 이후 결과 리포트 품질이 좋아질 가능성이 높다.
- LLM 요약이 실패해도 진단 흐름이 끊기지 않도록 규칙 기반 fallback이 필요하다.

개발/운영 관점:

- 실제 청약홈/마이홈 PDF는 페이지 수와 표가 많아 전체 표 추출 비용이 크다.
- PDF 원본은 저장하지 않는 기존 원칙은 유지한다.
- API 계약 변경은 문서에 남겨 React/Django/FastAPI 연결부가 같은 기준을 보게 한다.

## 4. 요구사항 정리

- PDF 텍스트 추출 시간을 줄인다.
- 사용자에게 raw 추출문이 아니라 의미 있는 검토용 공고 요약을 보여준다.
- 진단 입력에는 불필요한 잡음을 줄인 정리/요약 공고문을 사용한다.
- LLM 요약/정리를 반드시 붙이되, 실패 시에도 규칙 기반 정리 결과로 계속 진행한다.
- 추후 마이페이지에서 “어떤 공고를 넣었고, 그때 기준 진단/점수/레포트가 무엇이었는지” 볼 수 있게 저장 흐름을 보강한다.

## 5. 이번 작업 범위

- PDF 분석 응답에 정리/요약 필드를 추가한다.
- React PDF 화면은 정리된 공고문을 기본 미리보기/수정 대상으로 보여준다.
- 전략 진단에는 PDF 원문 추출문보다 정리된 공고문을 우선 전달한다.
- `pdf_analysis_id`를 전략 진단 요청까지 전달해 이력 추적성을 높인다.
- 문서/API 계약을 변경사항에 맞춰 갱신한다.

## 6. 1차 변경 후 PDF 처리 로직

2026-07-07 1차 구현 후 로직이다.

```text
React PdfAnalysis.tsx
  -> api.analyzePdf(file)
  -> Django PDFAnalyzeAPIView
      - 인증 필요
      - PDF 확장자/MIME 검사
      - 15MB 제한
      - 원본 저장 없이 bytes read
  -> FastAPI /api/pdf/analyze
      - PDF 확장자/MIME 검사
      - 빈 파일/15MB/%PDF 헤더 검사
  -> Backend/app/services/pdf_service.py
      - pdfplumber로 PDF 앞쪽 핵심 페이지 텍스트 추출
      - 초반 페이지 표를 제한적으로 추출
      - 텍스트가 500자 미만이면 PyMuPDF fallback
      - 규칙 기반으로 청약 핵심 항목/중요 문장을 정리
      - OPENAI_API_KEY가 있으면 LLM으로 사용자 검토용/진단 입력용 요약 생성
      - LLM 실패 시 규칙 기반 정리본 사용
  -> React에서 diagnosis_text를 편집 가능한 정리본으로 표시
  -> 원문 추출 일부는 raw_preview로 접어서 표시
  -> 전략 진단 버튼 클릭
  -> StrategyRun.tsx state로 확인/수정한 정리본과 pdf_analysis_id 전달
  -> POST /api/strategy announcement_text로 전송
  -> Django StrategyRun input_snapshot/result_payload 저장
  -> FastAPI /api/announcement가 Node4 LLM 구조화 후 상세 진단 실행
```

### 1차 변경 응답 필드

기존 필드는 유지하고 아래 필드를 추가했다.

```text
raw_preview
summary_text
diagnosis_text
summary_source
```

`combined_text`는 하위 호환 필드로 유지하되, 현재는 `diagnosis_text`와 같은 정리본을 담는다.

## 7. 검증 기록

- FastAPI import OK
- Django `manage.py check` OK
- Django `accounts strategy` tests 27 passed
- React tests 7 passed
- React production build OK
- 메모리 생성 PDF로 `analyze_pdf_bytes` 직접 호출해 새 응답 필드 생성 확인
- 단, 로컬 생성 PDF는 한글 폰트 추출이 깨져 실제 청약홈/마이홈 PDF 기준 품질 검증이 추가로 필요하다.
- 2차 구현 중 실제 PDF `2026000153 동작 센트럴 동문 디 이스트 입주자모집공고문.pdf`로 직접 검증했다.
  - 공고명: 동작 센트럴 동문 디 이스트 입주자모집공고
  - 공급 위치: 서울특별시 동작구 상도동 363-10번지 일원
  - 주택형/전용면적: 46, 51, 53, 56A, 56B, 58A, 58B, 58C, 59, 62 총 10개 주택형 추출
  - 공급금액 범위: 약 12.06억~17.07억 추출
  - 청약 일정/재당첨 제한/전매 제한/거주의무 추출
- 2차 구현 후 검증:
  - FastAPI import OK
  - Django `manage.py check` OK
  - Django `accounts strategy` tests 27 passed
  - React tests 7 passed
  - React production build OK

## 8. 남은 과제

- 실제 청약홈/마이홈 PDF 샘플로 추출 시간과 요약 품질을 확인한다.
- `OPENAI_API_KEY`가 있는 환경에서 LLM 요약 결과 품질을 확인한다.
- 마이페이지/결과 상세에서 정리된 공고문 요약과 진단 레포트가 함께 잘 보존되는지 점검한다.
- 필요하면 `StrategyRun.input_snapshot.announcement`에 PDF 요약 관련 메타데이터를 더 명확히 저장한다.
- 구조화 공고 필드(`announcement_name`, `region`, `sale_price_krw`, 일정 등)를 PDF 요약 단계에서 별도 필드로 뽑을지 검토한다.
- 마이페이지/결과 상세 제목은 공고가 있으면 아파트명/공고명, 공고가 없으면 `청약 가능성 분석`으로 보여주는 방향을 검토한다.

## 9. 2차 개선 로드맵: 짧은 사용자 요약 + 구조적 진단 입력

사용자 피드백 기준으로, PDF 요약은 공고문 대체본이 아니라 여러 공고를 비교하고 나중에 다시 식별하기 위한 짧은 핵심 카드 역할이면 충분하다고 판단했다. 반면 진단 입력은 Node4/LLM이 안정적으로 읽을 수 있도록 구조적이어야 한다.

### 왜 바꾸는가

- 마이페이지/결과 상세에는 “내가 어떤 공고를 넣었는지”만 빠르게 확인하면 되므로 장문 요약은 오히려 방해가 된다.
- 청약 가능성 검토에는 주택형/전용면적/공급금액 범위처럼 표 기반 필수 정보가 중요하다.
- 1차 개선 결과에서 공고명, 위치, 일정, 제한사항은 잘 잡혔지만 전용면적/분양가가 `확인 필요`로 남았다. 원문 7~8페이지 표에는 실제 값이 있으므로 표 기반 규칙 추출을 강화해야 한다.
- LLM은 최종 문장을 다듬는 보조 역할로 두고, 필수값은 가능한 한 규칙 기반으로 먼저 확보해야 한다.

### 구현 순서

1. `pdf_service.py`에 `extracted_fields`를 만든다.
   - 공고명, 공급 위치, 주택 유형, 규제지역 여부
   - 모집공고일, 청약 일정, 재당첨/전매/거주의무
   - 공급 세대수
   - 주택형/전용면적 목록
   - 공급금액 최저~최고 범위
2. `summary_text`와 `diagnosis_text`의 역할을 분리한다.
   - `summary_text`: 사용자 표시/이력용 짧은 핵심 요약
   - `diagnosis_text`: 전략 진단 입력용 구조화 텍스트
3. React에서 PDF 분석 결과를 전략 진단으로 넘길 때 `pdf_summary_text`, `pdf_extracted_fields`도 함께 넘긴다.
4. Django `input_snapshot.announcement`에 PDF 메타데이터를 저장한다.
5. 실제 동작 센트럴 동문 디 이스트 PDF로 전용면적/분양가 범위가 잡히는지 검증한다.

### 2차 구현 결과

- `extracted_fields`를 PDF 분석 응답에 추가했다.
- `summary_text`는 사용자 이력/확인용 짧은 핵심 요약으로 만들었다.
- `diagnosis_text`는 전략 진단 입력용 구조화 텍스트로 만들었다.
- `combined_text`는 하위 호환을 위해 유지하되 `diagnosis_text`와 같은 값을 담는다.
- React에서 PDF 분석 결과를 전략 진단으로 넘길 때 `pdf_summary_text`, `pdf_extracted_fields`도 함께 넘기도록 연결했다.
- Django는 새 PDF 메타데이터를 `StrategyRun.input_snapshot.announcement`에 저장한다.

2차 개선의 핵심 이유:

- 사용자에게는 공고문 전체 요약보다 “공고 식별과 비교에 필요한 짧은 요약”이 더 적합하다.
- 진단에는 자연어 장문보다 주택형/전용면적/공급금액/일정/제한사항이 구조적으로 정리된 입력이 더 적합하다.
- 실제 PDF에서 LLM이 표 기반 값을 놓칠 수 있으므로, 주택형과 공급금액 범위는 규칙 기반으로 먼저 추출한다.

## 10. 추적 로그

- 2026-07-07: 작업 시작. 현재 로직 확인 및 개선 방향 확정.
- 2026-07-07: FastAPI PDF 분석에 페이지/표 추출 제한, 규칙 기반 공고문 정리, 선택적 LLM 요약을 추가.
- 2026-07-07: React PDF 화면을 정리본 편집 중심으로 변경하고 `pdf_analysis_id`를 전략 진단 요청까지 전달하도록 연결.
- 2026-07-07: API 계약 문서에 `raw_preview`, `summary_text`, `diagnosis_text`, `summary_source` 필드 추가.
- 2026-07-07: 추적 파일을 baseline/개선 필요성/1차 변경 후 로직/검증 기록 구조로 재정리.
- 2026-07-07: 2차 개선 방향을 “짧은 사용자 요약 + 구조적 진단 입력 + 표 기반 필수 필드 추출”로 확정.
- 2026-07-07: 2차 구현으로 `extracted_fields`, 짧은 `summary_text`, 구조적 `diagnosis_text`, PDF 메타데이터 저장 흐름을 추가.
- 2026-07-07: 실제 동작 센트럴 동문 디 이스트 PDF로 주택형 10개와 공급금액 범위 추출을 검증.
- 2026-07-07: Django/FastAPI/React 검증 통과를 기록.
- 2026-07-07: 마이페이지/결과 상세 표시 로직을 보강했다. PDF 기반 진단은 `pdf_extracted_fields.announcement_name`을 우선 제목으로 사용하고, 공고 없는 진단은 `청약 가능성 분석`으로 통일했다. 시간은 기존 `created_at` 별도 표시를 유지해 제목이 길어지지 않게 했다.
- 2026-07-07: 전략 진단 화면에서 바로 PDF 파일 선택/드래그가 가능하도록 연결했다. PDF 처리 로딩 UI는 기존 `ProcessingIndicator`를 재사용하고, 분석된 정리본과 PDF 메타데이터는 브라우저 탭 세션에 임시 저장해 다른 탭에 다녀와도 사라지지 않게 했다.
- 2026-07-07: 긴 정책 안내문이 공고 제목으로 잡히는 케이스를 줄이기 위해 제목 정리 규칙을 보강했다.
- 2026-07-07: 원격 브랜치에 추가된 프론트 진행 UI를 확인한 뒤 전략 진단 화면의 PDF 진입점을 하나로 줄였다. 상단 버튼과 내부 파일 선택 버튼이 동시에 보여 혼란이 있었고, 드래그 드롭 이벤트가 안정적으로 전달되지 않아 업로드 영역 전체를 클릭/드롭 가능한 단일 영역으로 정리했다.
- 2026-07-07: PDF 추출 중/최종 진단 중 진행창은 `현재 지원 범위` 안내 아래가 아니라 공고 입력 카드 내부에서 입력 영역을 대체하도록 조정했다. 사용자가 현재 작업 중인 맥락을 잃지 않게 하고, 로딩 중 불필요한 입력 UI를 숨기기 위한 변경이다.
- 2026-07-07: PDF 드래그 중에는 `아파트 분양 모집공고 입력` 카드 전체가 드롭 영역처럼 보이도록 오버레이를 추가했다. 사용자가 작은 업로드 박스를 정확히 겨냥하지 않아도 카드 안 어디든 놓을 수 있게 하기 위한 UX 보강이다.
- 2026-07-07: PDF 추출 로딩과 최종 진단 로딩의 시각 구조를 통일했다. 현재 백엔드가 단계별 진행 이벤트를 보내지 않으므로 진행률 바는 실측값이 아니라 `파일 확인 -> 내용 추출 -> 결과 정리`, `프로필 확인 -> 공고 조건 비교 -> 전략 정리` 단계별 시간 추정값으로 표시한다.
- 2026-07-07: 드래그 이벤트는 카드 전체 영역만 담당하도록 정리했다. 내부 업로드 박스의 중복 drag handler를 제거해 enter/leave depth 계산이 흔들릴 여지를 줄였다.
- 2026-07-07: 레포트 제목은 표시 시점 계산만 하지 않고 `input_snapshot.announcement.display_title`로 명시 저장하도록 보강했다. PDF 기반 진단은 추출된 공고명/파일명/입력 텍스트 순서로 제목을 정하고, 공고 없는 진단은 `청약 가능성 분석`으로 저장한다.
- 2026-07-07: 마이페이지 기록 목록을 카드형으로 개편했다. 공고 기반 진단은 `display_title`과 위치 기준으로 그룹핑해 총 분석 횟수/최근 분석일/간단 위치를 보여주고, `분석 이력 보기` 모달에서 같은 공고의 분석 이력을 최근순으로 선택하도록 했다. 일반 진단은 단일 카드로 유지했다.
- 2026-07-07: 카드 핵심 태그에서 `PDF 공고` 같은 중복 표기는 제외했다. 제목이 아파트명인 경우 공고 기반임이 충분히 드러나므로 카드에는 날짜, 횟수, 위치처럼 비교에 도움이 되는 정보만 남겼다.
- 2026-07-07: 공고 기반 그룹 카드를 카드팩/스택형 대표 카드로 개선했다. 대표 카드 뒤에 겹친 이력 카드를 시각적으로 배치하고, 앞 카드에는 최근 분석 미리보기 3개를 보여줘 “한 공고에 여러 분석이 쌓였다”는 맥락이 더 잘 드러나도록 했다.
- 2026-07-07: 마이페이지 기록 영역을 가로 카드 레일로 바꾸고, 마우스 휠 세로 입력도 가로 스크롤로 변환했다. 공고 그룹 카드를 선택하면 dim 배경 위에서 카드가 중앙으로 확대/flip-in 되며, 같은 공고의 분석 내역을 최근순 카드로 선택할 수 있게 했다.
- 2026-07-07: 카드 레일의 가로 스크롤바를 숨기고, 레일 포커스 상태에서 `←/→`, `Home/End`, `PageUp/PageDown`으로 이동할 수 있게 했다. 무한 스크롤은 과한 구현 범위로 보고, 좌우 끝에는 AFIT 카드백을 배치해 레일의 시작/끝이 시각적으로 자연스럽게 보이도록 했다.
- 2026-07-07: 마이페이지 레포트 카드는 실제 가로 스크롤 레일이 아니라 3장 고정 인덱스 캐러셀로 수정했다. 의도한 UX가 “항상 3장이 보이고 휠/트랙패드/키보드 입력으로 한 장씩 넘어가는 카드팩”에 가까웠기 때문이다. 가로 스크롤바는 노출하지 않고, 좌우 끝에는 AFIT 카드백을 배치했다. 공고 기반 카드는 dim/flip-in 이력 모달을 열고, 기본 조건 분석 카드는 상세 레포트로 바로 진입한다.
- 2026-07-07: 양끝 AFIT 카드백은 코드로 그린 임시 스타일을 제거하고 정적 이미지 `/afit-card-back.png`만 표시하도록 바꿨다. 카드 크기는 기존 공고 카드와 동일한 높이/너비에 맞춘다. 카드 영역 휠 입력은 React `onWheel` 대신 native `wheel` listener를 `passive: false`로 등록해, 페이지 스크롤과 카드 넘김이 함께 발생하지 않도록 분리했다.
- 2026-07-07: 제공된 AFIT 카드백 이미지를 `public` 자산으로 연결했다. 모바일 카드에는 `afit-card-back_270x390.png`, 데스크톱 카드에는 `afit-card-back_300x430.png`를 `<picture>`로 선택해 원본 비율과 카드 크기를 그대로 맞춘다. 복잡한 카드백 질감/음영은 CSS 재현보다 이미지 사용이 더 적합하다고 판단했다.
- 2026-07-07: 공고 기반 레포트 카드는 AFIT 카드백과 이어지도록 아이보리 카드팩 톤으로 단순화했다. 카드 밖에서는 아파트명, 위치, 레포트 생성 횟수, 최근 생성 일시만 보여주고, 개별 분석 미리보기는 모달로 넘겼다. 이력 모달은 유지하되 고정 높이 안에서 오른쪽 이력 목록만 스크롤되도록 바꿔 분석 항목이 많아져도 화면이 아래로 계속 늘어나지 않게 했다.
- 2026-07-07: `공고문_영천해피포유...pdf`처럼 공고명이 비거나 placeholder인 PDF에서 레포트 제목이 `[아파트 청약 진단용`으로 저장/표시되는 문제를 확인했다. 원인은 진단 입력용 정리본의 첫 줄 `[아파트 청약 진단용 공고문 정리]`가 제목 후보로 채택된 것이다. Django 저장 로직과 React 표시 로직 모두에서 진단용 헤더/`공고명: 확인 필요` placeholder를 제목 후보에서 제외하고, 파일명 후보는 `공고문_` 접두어를 제거해 `영천해피포유`로 fallback되도록 보강했다.
- 2026-07-07: 기본 조건 분석은 공고 카드 캐러셀에서 제외하고 별도 섹션으로 분리했다. 동일 프로필 스냅샷은 화면에서 하나의 그룹으로 묶고, 횟수는 표시하지 않으며 최근 분석 일시와 대표 태그(지역, 무주택/주택보유, 세대주/세대원, 청약통장)를 보여준다. DB는 현재 `StrategyRun` 한 건이 입력 스냅샷과 결과를 함께 저장하는 구조라 “동일 조건이면 날짜만 추가 저장”을 하려면 별도 히스토리 모델/중복 정책이 필요하므로 이번 작업에서는 표시 계층 그룹핑으로 처리했다.
- 2026-07-08: 마이페이지 상단에 `현재 기준 프로필` 요약 바를 추가했다. 기본 조건 분석을 공고 카드와 섞지 않되, 사용자가 “지금 내 조건이 무엇이었는지”를 먼저 확인하고 프로필 수정/기본 조건 분석으로 바로 이동할 수 있게 하기 위한 변경이다.
- 2026-07-08: 같은 공고의 분석 이력 모달에 `조건 변경` 배지를 추가했다. 최신 분석과 직전 분석의 프로필 스냅샷을 비교해 거주지, 무주택 여부, 무주택 기간, 세대주 여부, 납입 횟수, 예치금, 자녀 수처럼 결과에 영향을 줄 수 있는 핵심 조건 변화만 짧게 보여준다.
- 2026-07-08: 결과 상세 화면에 `PDF로 저장` 버튼과 면책 조항을 추가했다. 다운로드는 브라우저 인쇄 기능(`window.print`)을 우선 사용해 별도 PDF 생성 라이브러리 없이 현재 레이아웃을 저장할 수 있게 하고, 레포트가 참고용 진단이며 최종 자격은 공식 공고/청약홈/사업주체 확인이 필요하다는 안내를 명시했다.
- 2026-07-08: 프로필 입력의 금액 필드를 프론트에서는 만원 단위로 입력하도록 바꿨다. 사용자는 `240`처럼 입력하고, 저장 직전에는 기존 백엔드 계약을 유지하기 위해 원 단위(`2,400,000`)로 변환한다. DB/API 단위를 바꾸지 않아 기존 분석 로직과 저장 데이터 호환성을 유지한다.
- 2026-07-08: 배포 시 Vite dev proxy가 아니라 Nginx가 업로드 요청을 먼저 받으므로 `deploy/nginx/default.conf`를 추가했다. Nginx `client_max_body_size`는 multipart 오버헤드를 고려해 20MB로 두고, 실제 서비스 정책은 기존 Django PDF 검증의 15MB 제한을 유지한다.
- 2026-07-08: 마이페이지의 기본 조건 분석 중복 섹션을 제거했다. 기본 조건은 상단 `기본 정보 진단` 카드에서 현재 프로필 태그, 최근 기본 분석일, 분석 보기/실행, 프로필 수정만 제공하도록 줄여 정보량을 낮췄다. 공고 기반 기록 영역에는 공고 카드만 남기고, 공고 기반 기록이 없을 때는 별도 빈 상태 안내를 보여준다.
- 2026-07-08: `기본 정보 진단` 카드는 설명문과 큰 강조 버튼을 줄이고, 제목/기준일/최대 4개 태그/작은 액션만 남기는 형태로 재정리했다. 마이페이지 첫 화면에서 공고 카드보다 기본 조건 카드가 더 무겁게 보이지 않도록 시각적 우선순위를 낮추기 위한 변경이다.
- 2026-07-08: `sample_pdfs` 5개로 PDF 추출/요약 품질을 재검증했다. 기존 2개 샘플 외 거제 민영분양, 의왕 국민임대, 군포 분양전환공공임대까지 넣어보니 일반 분양 본문에 포함된 `분양전환` 문구 때문에 민영 공고가 임대주택 범위 밖으로 오분류되고, 임대조건 섹션이 없는 공고의 계약금/중도금이 임대보증금처럼 잡히며, 통합 LH 공고의 제목이 너무 일반적으로 잡히는 문제가 확인됐다.
- 2026-07-08: 위 문제를 줄이기 위해 공고 유형 판정은 파일명/정리된 제목/초반 공급대상 중심으로 제한하고, 임대조건은 실제 `임대조건/임대보증금/월임대료` 섹션이 있을 때만 추출하도록 바꿨다. 주소 추출은 도로명/번지 주소를 먼저 잡고, LH 통합 공고는 파일명 기반 제목과 지역 fallback을 사용한다. 국민임대 표의 `주택형/예비자수` 행, 미분양 매각 표의 `공급호수/전용면적/주소` 행, 공공임대의 `전용면적 55㎡ 52세대` 행도 별도 패턴으로 보강했다.
- 2026-07-08: PDF 처리 시간도 같이 조정했다. 전체 40쪽/표 15쪽 스캔은 군포 샘플에서 160초 이상 걸렸고, 핵심값은 앞쪽에 몰려 있었으므로 기본 스캔 범위를 16쪽/표 8쪽/표 10개로 낮췄다. 최종 샘플 기준 동작 37초, 거제 19초, 의왕 14초, 영천 12초, 군포 50초 수준으로 확인했으며, 동작/거제는 민영분양, 의왕/군포는 현재 서비스 진단 범위 밖 임대계열, 영천은 선착순 매각 공고로 구분된다.
- 2026-07-08: 청약홈 일정표가 `서류접수/계약체결` 기간을 시작일 줄과 종료일 줄로 나눠 추출하는 경우를 보정했다. 거제 샘플에서 계약 기간이 `2026.08.01 ~ 2026.08.11`처럼 섞였던 문제를 확인했고, 일정표 주변 3줄을 함께 읽어 서류 제출 `2026.08.01 ~ 2026.08.07`, 계약 `2026.08.11 ~ 2026.08.13`으로 매핑되도록 테스트를 추가했다.
