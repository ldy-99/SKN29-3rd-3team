...
A-FIT은 기존 FastAPI/LangGraph/RAG 기반 청약 AI 진단 엔진을 실제 사용자가 접근 가능한 웹서비스로 확장한 프로젝트다. React는 사용자 화면, Django는 인증/세션/저장/공개 API 경계 및 Gunicorn 기반 WSGI 웹 애플리케이션 처리를 담당하고, FastAPI는 AI 진단/PDF/RAG 처리를 담당한다.
...
flowchart LR
    ...
    R -->|"Django 공개 API /api/*"| D["Django REST API (Gunicorn)"]
    D --> S[("SQLite DB")]
...
| UC-04 | 공고문 직접 입력 진단 | 텍스트 공고문 입력 후 상세 진단 | `/strategy`, `POST /api/strategy` | 완료 (검증 완료) |
...
| FR-STRATEGY-02 | 공고문 텍스트 기반 진단을 실행할 수 있어야 한다 | 완료 | 브라우저 수동 검증 PASS |
| FR-STRATEGY-03 | 진단 결과는 이력에 저장되어야 한다 | 완료 | `StrategyRun` 모델 저장 |
...
| ID | 요구사항 | 상태 | 구현/검증 |
|---|---|---|---|
| FR-PDF-01 | PDF 파일을 업로드해 텍스트/표를 추출할 수 있어야 한다 | 완료 | PDF 파싱 서비스 및 Nginx 20MB 업로드 검증 |
| FR-PDF-02 | LLM 요약 또는 규칙 기반 요약으로 공고문을 정리해야 한다 | 완료 | summary_source 연동 |
...
| NFR-SEC-02 | 보안 | 운영 환경 secure cookie/HTTPS는 분리 관리해야 한다 | 완료. HTTP 전용 설정 추가 완료 |
| NFR-PERF-01 | 안정성 | 장시간 요청은 timeout 또는 오류 안내를 제공해야 한다 | 완료, 예외 처리 설계 적용 |
| NFR-DEP-01 | 배포 | Docker Compose 기반 실행 구조를 제공해야 한다 | 완료. 실제 이미지 빌드 및 EC2 배포/구동 검증 완료 |
| NFR-DATA-01 | 데이터 | SQLite 데이터는 Docker volume으로 유지할 수 있어야 한다 | 완료. 볼륨 설정 및 컨테이너 재기동 후 데이터 보존 검증 완료 |
...
| LLM-REQ-03 | PDF 요약 실패 시 진단 흐름이 중단되지 않아야 한다 | 규칙 기반 정리본 유지 | 완료 |
| LLM-REQ-04 | RAG 검색 실패가 전체 서비스 실패로 이어지지 않아야 한다 | found=false 또는 안내 응답 | 완료 |
