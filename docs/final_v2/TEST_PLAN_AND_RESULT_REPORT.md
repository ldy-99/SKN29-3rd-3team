...
| 평가 요소 | 본 문서의 확인 범위 |
|---|---|
| 기능 테스트 커버리지 | 회원 인증, 청약 프로필, 전략 진단, PDF 분석, 결과 상세, 마이페이지, Floating 챗봇 |
...
| MAN-FE-04 | 챗봇 | Floating 챗봇 제어 | 우하단 챗봇 버튼 클릭 및 닫기 | 패널 열림/닫힘, 가로 overflow 없음 | 정상 작동, 모바일 뷰포트 유지 | PASS |
...
| TC-CHAT-01 | 챗봇 | RAG 질문/답변 | 질문에 대한 챗봇 답변 수신 | RAG 기반 맞춤형 청약 지식 답변 수신 | PASS | OpenAI API 실연동 확인 |
| TC-CHAT-02 | 챗봇 | 출처 표시 | 답변 데이터 하단 출처 표기 | 참고한 법령/공고문 정보 정상 노출 | PASS | |
...
|---|---|---|---|---|
| LLM-01 | 정상 응답 | 외부 LLM 호출 후 데이터 요약 가공 성공 | AI 청약 진단 및 챗봇 답변 성공 | PASS |
...
* **CI/CD 파이프라인 자동화:** Git Push 발생 시 GitHub Actions를 통해 도커 빌드, 허브 푸시, EC2 원격 재기동까지 일괄 처리되는 자동 배포망 구축.
